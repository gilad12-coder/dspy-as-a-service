...
import {
  ...
  getTaggerSessionSharing,
  putTaggerSessionSharing,
  removeTaggerSessionShareMember,
  searchUsers,
  transferTaggerSessionOwnership,
  updateTaggerSessionShareMember,
  type TaggerSessionSharingState,
  type GeneralAccess,
  type LinkRole,
  type MemberRole,
  type ShareRole,
...
/**
 * Drive-style sharing modal for a saved labeling session. The session twin of
 * :func:`DatasetShareDialog` — a People section (invite by username + per-member
 * role) over a General-access section (Restricted vs Anyone-with-link) plus a
 * copy-link row. The copy link points at ``/tagger/share/{token}``, redeemed by
 * :mod:`app/tagger/share/[token]`.
 */
export function TaggingSessionShareDialog({ sessionId }: { sessionId: string }) {
  const { data: session } = useSession();
  const me = sessionIdentity(session);
  const [open, setOpen] = useState(false);
  const [state, setState] = useState<TaggerSessionSharingState | null>(null);
  const [savingAccess, setSavingAccess] = useState(false);
  const [transferTarget, setTransferTarget] = useState<string | null>(null);
  const [transferring, setTransferring] = useState(false);
  // Split the confirm copy around the {name} token so the target username can
  // render emphasized below in an isolated <bdi>, matching the sibling dialogs.
  ...
  const shareUrl =
    state?.token && typeof window !== "undefined"
      ? `${window.location.origin}/tagger/share/${state.token}`
      : null;
  ...
  const handleOpenChange = (next: boolean) => {
    setOpen(next);
    if (next && state === null) {
      getTaggerSessionSharing(sessionId)
        .then(setState)
  ...
  const handleLinkRoleChange = async (role: LinkRole) => {
    ...
    try {
      setState(
        await putTaggerSessionSharing(sessionId, { general_access: "anyone", general_role: role }),
      );
      toast.success(msg("share.access_updated"));
    } catch (err) {
      toast.error(err instanceof Error ? err.message : msg("share.save_failed"));
  ...
  const handleRoleChange = async (username: string, role: MemberRole) => {
    try {
      setState(await updateTaggerSessionShareMember(sessionId, username, { role }));
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (!alreadyMember) {
        await addTaggerSessionShareMember(sessionId, { username: transferTarget, role: "editor" });
      }
      await transferTaggerSessionOwnership(sessionId, transferTarget);
  ...
  const handleRemove = async (username: string) => {
    try {
      setState(await removeTaggerSessionShareMember(sessionId, username));
