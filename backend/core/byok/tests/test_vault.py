...
in-memory SQLite engine with the BYOK tables and patches ``httpx.get`` so no
...
from core.storage.models import Base, ByokProviderKeyModel
...
def test_save_key_accepts_unknown_provider(engine: object, vault_key: str) -> None:
    """Store arbitrary provider slugs for provider-agnostic pass-through."""
    vault = ProviderKeyVault(engine=engine)
    view = vault.save_key("u@x.com", "private-provider", "sk-1234")

    assert view.provider == "private-provider"
    assert view.status == STATUS_UNVERIFIED
...
def test_save_key_custom_api_base_probes_that_endpoint(engine: object, vault_key: str) -> None:
    ...
    with patch("core.byok.vault.httpx.get", side_effect=fake_get):
...
def test_save_key_unknown_provider_without_api_base_is_unverified(
    engine: object,
    vault_key: str,
) -> None:
    """Keep an opaque connection even when it has no discoverable endpoint."""
    vault = ProviderKeyVault(engine=engine)
    view = vault.save_key("u@x.com", "custom", "sk-1234")
