...
from core.api.routers.byok import _catalog_for_user
...
def test_custom_catalog_fetches_models_with_stored_secret(vault: ProviderKeyVault) -> None:
    ...
    with (
        patch(
            "core.api.routers.byok.get_byok_catalog_cached",
            return_value=ModelCatalogResponse(providers=[], models=[]),
        ),
        patch(
            "core.api.routers.byok.discover_models_at_endpoint",
            return_value=discovered,
        ) as discover,
    ):
        catalog = _catalog_for_user(vault, "alice@example.com")
...
def test_custom_catalog_is_scoped_to_the_authenticated_account(
    ...
    with patch(
        ...
        catalog = _catalog_for_user(vault, "bob@example.com")
